...
function fallbackQueries({ keyword, industry, country, region, discoveryRound = 0 }) {
  ...
  const queryFamilies = [
    ...
    [
      ...
      `${base} sponsor email`,
      `${base} conference speaker email`,
      `${base} association member directory`,
      `${base} portfolio companies contact`,
      `${base} startup directory email`,
    ],
    [
      `${base} community contact`,
      `${base} advisory board email`,
      `${base} agency founder`,
      `${base} about us email`,
      `${base} investor relations email`,
      `${base} press email`,
    ],
  ];
  const stems = queryFamilies[discoveryRound % queryFamilies.length] || queryFamilies[0];
...
function buildAdaptiveFallbackTargets(keyword, plannedQueries = []) {
  const q = encodeURIComponent(keyword);
  const queryTargets = plannedQueries.slice(0, 3).flatMap((query) => {
...
export async function discoverSeedUrls(config, fetcher) {
  ...
  if (discovered.size === 0) {
    for (const fallbackUrl of buildAdaptiveFallbackTargets(keyword, plannedQueries)) {
      discovered.add(fallbackUrl);
    }
  }
