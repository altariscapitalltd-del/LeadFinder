...
function getEnvProviders() {
  const envProviders = [];
  if (isUsableApiKey(process.env.ANTHROPIC_API_KEY)) envProviders.push({ provider: "anthropic", provider_type: "anthropic", api_key: process.env.ANTHROPIC_API_KEY, active: 1 });
  if (isUsableApiKey(process.env.OPENAI_API_KEY)) envProviders.push({ provider: "openai", provider_type: "openai", api_key: process.env.OPENAI_API_KEY, active: 1 });
  if (isUsableApiKey(process.env.OPENROUTER_API_KEY)) envProviders.push({ provider: "openrouter", provider_type: "openai", api_key: process.env.OPENROUTER_API_KEY, base_url: "https://openrouter.ai/api/v1", active: 1 });
  if (isUsableApiKey(process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY)) envProviders.push({ provider: "gemini", provider_type: "gemini", api_key: process.env.GOOGLE_API_KEY || process.env.GEMINI_API_KEY, active: 1 });
  if (isUsableApiKey(process.env.GROQ_API_KEY)) envProviders.push({ provider: "groq", provider_type: "openai", api_key: process.env.GROQ_API_KEY, base_url: "https://api.groq.com/openai/v1", active: 1 });
  return envProviders;
}
...
function isUsableApiKey(value) {
  ...
  if (/change-this|your-|example/i.test(normalized)) return false;
  return normalized.length >= 12;
}

function getConfiguredProviders(sessionProviders = []) {
  let fromDb = [];
  try {
    const db = getDb();
    fromDb = db.prepare("SELECT * FROM ai_settings ORDER BY active DESC, updated_at DESC").all();
  } catch {
    fromDb = [];
  }
  const map = new Map();
  for (const item of [...sessionProviders, ...fromDb, ...getEnvProviders()]) {
...
export async function refreshProviderModels(provider, sessionProviders = []) {
  const setting = getConfiguredProviders(sessionProviders).find((item) => item.provider === provider);
...
async function ensureModelsLoaded(setting) {
  ...
  const cached = db
    ? db.prepare("SELECT * FROM ai_models WHERE provider = ? AND available = 1 ORDER BY updated_at DESC").all(setting.provider)
    : [];
...
async function getCandidates(taskType, sessionProviders = []) {
  let db = null;
  try {
    db = getDb();
  } catch {
  ...
  const settings = getConfiguredProviders(sessionProviders);
  const candidates = [];
  for (const setting of settings) {
    const models = await ensureModelsLoaded(setting);
    for (const model of models) {
      const stat = db
        ? db.prepare("SELECT * FROM ai_model_stats WHERE provider = ? AND model_id = ? AND task_type = ?")
...
export async function complete({ system = "", prompt = "", messages, maxTokens = 2000, taskType = "chat", temperature = 0.4, sessionProviders = [] }) {
  const preparedMessages = Array.isArray(messages) && messages.length
    ? messages
    : [{ role: "user", content: prompt }];
  const candidates = await getCandidates(taskType, sessionProviders);
...
export function listProviderStatus(sessionProviders = []) {
  let db = null;
  try {
    db = getDb();
  } catch {
  ...
  const providers = getConfiguredProviders(sessionProviders);
  return providers.map((provider) => {
    const models = db
      ? db.prepare("SELECT provider, model_id, label, speed_tier, quality_tier, context_window, updated_at FROM ai_models WHERE provider = ? AND available = 1 ORDER BY updated_at DESC").all(provider.provider)
      : (parseJsonSafe(provider.models_json, []) || []).map((modelId) => ({ provider: provider.provider, model_id: modelId, label: modelId }));
    const stats = db
      ? db.prepare("SELECT provider, model_id, task_type, total_requests, success_count, error_count, avg_latency_ms, updated_at FROM ai_model_stats WHERE provider = ? ORDER BY updated_at DESC LIMIT 20").all(provider.provider)
      : [];
    return {
...
export async function generateEmail({ goal, tone = "professional", sessionProviders = [] }) {
...
export async function scoreContact(contact, sessionProviders = []) {
  const text = await complete({
...
export async function generateReply({ originalEmail, senderName, tone = "professional", sessionProviders = [] }) {
  return complete({
