...
async function executeTool(call) {
  ...
  switch (call?.tool) {
    ...
    case "create_template": {
      ...
      const generated = await generateEmail({ goal: args.goal, tone: args.tone || "professional" });
      const result = db.prepare(`
...
async function planAgentReply(history, snapshot) {
...
${transcript}`,
    ...
    if (/scrape|harvest|find emails/.test(latest)) {
      return {
        ...
        toolCalls: [{ tool: "queue_scrape", arguments: { keyword: latest.replace(/.*(?:for|about)\s+/i, "").trim(), maxPages: 75 } }],
      };
    }
...
async function summarizeAfterTools(history, toolResults, snapshot) {
  const transcript = history.map((msg) => `${msg.role.toUpperCase()}: ${msg.content}`).join("\n\n");
  try {
    const text = await complete({
...
export async function runAgentTurn({ threadId, message }) {
  ...
  const plan = await planAgentReply(before, snapshot);
  ...
  const reply = toolResults.length
    ? await summarizeAfterTools(history, toolResults, getAppSnapshot())
