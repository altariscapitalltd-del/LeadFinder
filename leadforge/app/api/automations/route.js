...
export async function PATCH(req) {
  ...
  const db = getDb();
  if (active !== undefined) db.prepare("UPDATE automations SET active = ? WHERE id = ?").run(active, id);
  if (last_run) db.prepare("UPDATE automations SET last_run = ? WHERE id = ?").run(last_run, id);
  if (next_run) db.prepare("UPDATE automations SET next_run = ? WHERE id = ?").run(next_run, id);
